...
module.exports = {
  ...
  getFormActive: function (req, res) {
    const request = [req.id]
    c.query("SELECT * FROM `forms` WHERE `active`=?", request, { metadata: true, useArray: true }, function (err, rows) {
      if (err) {
        res.send({ message: err.message });
        console.log(err);
        return
  ...
  newForm: function (req, res) {
    ...
    c.query("INSERT INTO `forms`(`id`, `name`, `effective_date`, `pic`, `version`, `standard_level_id`, `active`, `file_pdf`, `file_doc`, `file_xls`, `file_pds`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", request, { metadata: true, useArray: true }, function (err, rows) {
      if (err) {
        res.send({ message: err.message });
        console.log(err);
        return
  ...
  updateForm: function (req, res) {
    ...
    c.query("UPDATE `forms` SET `name`=?, `effective_date`=?, `pic`=?, `version`=?, `standard_level_id`=?, `active`=?, `file_pdf`=?, `file_doc`=?, `file_xls`=?, `file_pds`=? WHERE `id`=?", request, { metadata: true, useArray: true }, function (err, rows) {
      if (err) {
        res.send({ message: err.message });
        console.log(err);
        return
